...
class InvisibleHandApp:
    ...
    def _show_slot_window(self, class_name: str, final_student: str):
        ...
        # NO BORDERS: only subtle fills + optional shadow
        card_fill = _mix(bg, fg, 0.02)
        band_fill = _mix(bg, primary, 0.06)

        # Optional soft shadow (still borderless). Delete these 2 lines if you want fully flat.
        ...
        # --- Name formatting: single-line only (prevents overlap) ---
        ...
        def _format_name(name: str) -> str:
            ...
            if " " in s:
                ...
                if has_cjk and has_latin:
                    left, right = s.split(" ", 11)
                    return f"{left} · {right}"
            return s

        # shrink-to-fit font cache
        ...
        # --- Progress + button row ---
        progress = ttk.Progressbar(win, mode="determinate", maximum=100, bootstyle="primary")
        ...
        def _finalize():
            ...
            # Borderless winner emphasis: change band FILL (not outline) + text color
            reel.itemconfig(band, fill=_mix(bg, success, 0.08), outline="")
            reel.itemconfig(t_cur, fill=success)

            # Clean readability: hide prev/next once selected
